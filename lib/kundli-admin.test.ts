...
describe("Kundli queue operational filters", () => {
  ...
  it("counts actual assignment states, statuses, risks, and conflicts", () => {
    const counts = countKundliQueueStates([item({ assignmentState: "AWAITING_ASSIGNMENT", conflict: true }), item({ id: "two", status: "IN_REVIEW", deliveryRisk: "DUE_SOON" }), item({ id: "three", status: "REPORT_READY", deliveryRisk: "OVERDUE" })]);
    expect(counts).toMatchObject({ awaiting: 1, inReview: 1, reportReady: 1, dueSoon: 1, overdue: 1, conflicts: 1 });
  });
});
