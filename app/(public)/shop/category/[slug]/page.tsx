...
import { CollectionCard, FilterChip, PremiumLink, PromoStrip, StorefrontSection, TrustFeatureStrip } from "@/components/storefront";
...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  const siblingCategories = category.parent
    ? categoryTree.find((item) => item.id === category.parentId)?.children ?? []
    : category.children;
  ...
  const heroImage = heroProduct ? productPrimaryImage(heroProduct) : null;
  ...
  return (
    <div className="grid gap-8 lg:gap-10">
        ...
        <div className="grid min-h-[420px] lg:grid-cols-[1.1fr_0.9fr]">
          <div className="flex flex-col justify-center px-5 py-9 sm:px-8 lg:px-10">
            ...
            <p className="mt-4 max-w-[300px] text-base leading-7 text-omd-muted sm:max-w-2xl">
              {category.description ?? categoryFallbackDescription(category.name)}
            </p>
            <div className="mt-5">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <PremiumLink href={filterHref(slug, filters, { sort: "rating" })} variant="secondary">View Best Sellers</PremiumLink>
            </div>
      ...
      {siblingCategories.length ? (
        <StorefrontSection
          eyebrow={category.parentId ? "Sibling subcategories" : "Subcategories"}
          title={category.parentId ? `More in ${parentCategory.name}` : `Shop within ${category.name}`}
          subtitle={category.parentId ? "Move quickly across related subcategories under the same parent." : "Choose a focused subcategory or keep browsing the full parent collection."}
        >
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {siblingCategories.map((child) => (
              <CollectionCard
                ...
                cta={child.id === category.id ? "Current Subcategory" : "Explore"}
                imageUrl={child.homepageIntentImage}
              />
            ))}
          </div>
        </StorefrontSection>
      ...
      <section className="grid gap-3 rounded-[1.5rem] border border-omd-sand bg-white/90 p-4 shadow-sm lg:grid-cols-[1fr_auto] lg:items-center">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-omd-saffron">Listing</p>
          <h2 className="mt-1 text-xl font-semibold text-omd-brown">{category.name} Products</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            {labels.length ? labels.map((label) => (
              ...
              <span className="rounded-full border border-omd-sand bg-omd-ivory px-3 py-1 text-xs font-semibold text-omd-muted">
                No active filters
              </span>
            )}
          </div>
      ...
      <div className="flex max-w-[320px] flex-wrap gap-2 sm:max-w-none">
        <FilterChip href={`/shop/category/${slug}`} selected={!hasFilters}>All</FilterChip>
        <FilterChip href={filterHref(slug, filters, { stock: "in" })} selected={filters.stock === "in"}>In Stock</FilterChip>
        <FilterChip href={filterHref(slug, filters, { featured: "true" })} selected={filters.featured === "true"}>Featured</FilterChip>
        <FilterChip href={filterHref(slug, filters, { sort: "rating" })} selected={filters.sort === "rating"}>Best Sellers</FilterChip>
        <FilterChip href={filterHref(slug, filters, { type: "KIT" })} selected={filters.type === "KIT"}>Kits</FilterChip>
        <FilterChip href={filterHref(slug, filters, { rating: "4" })} selected={filters.rating === "4"}>Highest Rated</FilterChip>
      </div>

      <div className="grid gap-6 xl:grid-cols-[310px_1fr]">
        <aside className="h-fit rounded-[1.5rem] border border-omd-sand bg-white p-5 shadow-sm xl:sticky xl:top-28">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-omd-saffron">Refine</p>
              <h2 className="mt-1 text-lg font-semibold text-omd-brown">Filters</h2>
            </div>
            <Link href={`/shop/category/${slug}`} className="text-xs font-semibold text-omd-saffron hover:text-omd-brown">Clear</Link>
          </div>
          ...
          <form className="mt-5 grid gap-5">
            ...
            <section className="grid gap-2">
              <label className="text-xs font-semibold uppercase tracking-wide text-omd-muted">Sort</label>
              <select name="sort" defaultValue={filters.sort ?? "featured"} className={inputClass()}>
                <option value="featured">Featured</option>
                <option value="newest">Newest</option>
                <option value="price-asc">Price low</option>
                <option value="price-desc">Price high</option>
            ...
            {filters.featured ? <input type="hidden" name="featured" value={filters.featured} /> : null}
            <button className="h-11 rounded-xl bg-omd-brown px-4 text-sm font-semibold text-white hover:bg-omd-saffron">
              Apply Filters
            </button>
          </form>

          <div className="mt-6 border-t border-omd-sand pt-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-omd-muted">Related Collections</p>
            <div className="mt-3 grid gap-1">
              {categoryTree.slice(0, 8).map((item) => (
                <div key={item.id} className="grid gap-1">
                  <Link
                    href={`/shop/category/${item.slug}`}
                    className={`rounded-xl px-3 py-2 text-sm font-semibold ${item.slug === slug ? "bg-omd-ivory text-omd-brown" : "text-omd-muted hover:bg-omd-ivory hover:text-omd-saffron"}`}
                  >
                    {item.name}
                  </Link>
                  {item.children.map((child) => (
                    <Link
                      key={child.id}
                      href={`/shop/category/${child.slug}`}
                      className={`rounded-xl px-3 py-1.5 pl-6 text-xs font-semibold ${child.slug === slug ? "bg-omd-ivory text-omd-brown" : "text-omd-muted hover:bg-omd-ivory hover:text-omd-saffron"}`}
                    >
                      {child.name}
                    </Link>
                  ))}
        ...
        <section id="products" className="grid gap-6 scroll-mt-28">
          {products.length === 0 ? (
            <EmptyState
              ...
              actions={
                <>
                  <PremiumLink href={`/shop/category/${slug}`}>Clear filters</PremiumLink>
