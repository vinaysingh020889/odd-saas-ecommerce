...
export default async function MyBenefitsPage() {
  ...
  const inProgress = redemptions.filter((item) => ["RESERVED", "CONSUMED"].includes(item.status) && !["COMPLETED", "DELIVERED", "CANCELLED", "REFUNDED"].includes(orderById.get(item.relatedId ?? "")?.status ?? ""));
  const used = redemptions.filter((item) => item.status === "CONSUMED" && ["COMPLETED", "DELIVERED"].includes(orderById.get(item.relatedId ?? "")?.status ?? ""));
  ...
  const history = (items: typeof redemptions) => items.length ? <div className="grid gap-3">{items.map((item) => { const order = orderById.get(item.relatedId ?? ""); return <div key={item.id} className="rounded-md border border-omd-sand p-3"><div className="flex flex-wrap justify-between gap-2"><p className="font-semibold text-omd-brown">{item.benefit.title}</p><StatusBadge tone={item.status === "CONSUMED" ? "success" : item.status === "RESERVED" ? "warning" : "neutral"}>{item.status}</StatusBadge></div><p className="mt-1 text-sm text-omd-muted">{order?.package.name ?? item.scope} · Saved {formatMoney(item.savingAmount)}</p>{order ? <Link className="mt-2 inline-flex text-sm font-semibold text-omd-saffron" href={`/kundli/${order.orderNo ?? order.id}`}>Track claim</Link> : null}</div>; })}</div> : <p className="text-sm text-omd-muted">Nothing here yet.</p>;
