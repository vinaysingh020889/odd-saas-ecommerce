...
export default async function MyBenefitsPage() {
  ...
  const serviceBookings = await prisma.serviceBooking.findMany({ where: { id: { in: relatedIds } }, include: { service: { select: { title: true } } } });
  const serviceById = new Map(serviceBookings.map((item) => [item.id, item]));
  const asthiApplications = await prisma.asthiApplication.findMany({ where: { id: { in: relatedIds } }, include: { package: { select: { name: true } } } });
  const asthiById = new Map(asthiApplications.map((item) => [item.id, item]));
  const fulfilmentStatus = (relatedId: string | null) => { const kundli = orderById.get(relatedId ?? ""); const shop = shopById.get(relatedId ?? ""); return kundli?.status ?? shop?.order.fulfillmentStatus ?? shop?.order.status ?? serviceById.get(relatedId ?? "")?.status ?? asthiById.get(relatedId ?? "")?.status ?? ""; };
  ...
  const history = (items: typeof redemptions) => items.length ? <div className="grid gap-3">{items.map((item) => { const order = orderById.get(item.relatedId ?? ""); const shopItem = shopById.get(item.relatedId ?? ""); const service = serviceById.get(item.relatedId ?? ""); const asthi = asthiById.get(item.relatedId ?? ""); return <div key={item.id} className="rounded-md border border-omd-sand p-3"><div className="flex flex-wrap justify-between gap-2"><p className="font-semibold text-omd-brown">{item.benefit.title}</p><StatusBadge tone={item.status === "CONSUMED" ? "success" : item.status === "RESERVED" ? "warning" : "neutral"}>{item.status}</StatusBadge></div><p className="mt-1 text-sm text-omd-muted">{order?.package.name ?? shopItem?.product.title ?? service?.service.title ?? asthi?.package?.name ?? item.scope} · Saved {formatMoney(item.savingAmount)}</p>{order ? <Link className="mt-2 inline-flex text-sm font-semibold text-omd-saffron" href={`/kundli/${order.orderNo ?? order.id}`}>Track claim</Link> : shopItem ? <Link className="mt-2 inline-flex text-sm font-semibold text-omd-saffron" href={`/orders/${shopItem.order.id}`}>Track claim</Link> : service ? <Link className="mt-2 inline-flex text-sm font-semibold text-omd-saffron" href={`/service-bookings/${service.bookingNo ?? service.id}`}>Track claim</Link> : asthi ? <Link className="mt-2 inline-flex text-sm font-semibold text-omd-saffron" href={`/asthi/${asthi.applicationNo ?? asthi.id}`}>Track claim</Link> : null}</div>; })}</div> : <p className="text-sm text-omd-muted">Nothing here yet.</p>;
