...
const navGroups: Array<{
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/admin/system-events", label: "System Events", icon: "settings", roles: ["SUPER_ADMIN", "OPERATIONS_ADMIN"] },
...
export default async function AdminLayout({
  ...
  const unreadNotifications = await prisma.notification.count({ where: { tenantId, recipientId: user.id, archivedAt: null, readAt: null } });
  const visibleGroups = navGroups
    .map((group) => ({ ...group, items: group.items.filter((item) => !item.roles || hasAnyRole(user, item.roles)) }))
    .map((group) => restrictedAstrologer ? { ...group, items: group.items.filter((item) => ["/admin/my-work", "/admin/notifications"].includes(item.href)) } : group)
    .filter((group) => group.items.length > 0);

  return (
    <div className="min-h-screen overflow-x-clip bg-slate-50 text-slate-950 lg:grid lg:grid-cols-[260px_minmax(0,1fr)] xl:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="hidden border-r border-slate-200 bg-slate-950 text-white lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {visibleGroups.map((group) => (
            <details key={group.label} open={!group.items.every((item) => runtimeConfig.phase1UatMode && !isPhase1AdminNavigationHref(item.href))} className="group mb-3 rounded-md open:bg-white/[0.03]">
              <summary className="cursor-pointer list-none rounded-md px-3 py-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-omd-gold"><span className="flex items-center justify-between gap-2"><span><span className="block text-xs font-semibold uppercase tracking-wide text-slate-400">{group.label}</span><span className="mt-0.5 block text-xs text-slate-500">{group.caption}</span></span><span aria-hidden="true" className="text-slate-500 transition group-open:rotate-180">v</span></span></summary>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
          <div className="mx-auto flex min-h-14 max-w-7xl items-center justify-between gap-3 px-4">
            ...
            <div className="flex items-center gap-3">
              <Link href="/admin/notifications" aria-label={`${unreadNotifications} unread notifications`} className="relative rounded-md border border-slate-300 p-2 text-slate-700 hover:border-omd-ops hover:text-omd-ops"><svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4"/></svg>{unreadNotifications ? <span className="absolute -right-2 -top-2 min-w-5 rounded-full bg-red-600 px-1 text-center text-xs font-bold leading-5 text-white">{unreadNotifications > 99 ? "99+" : unreadNotifications}</span> : null}</Link>
          ...
          <details className="border-t border-slate-100 lg:hidden">
            <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-slate-800 focus-visible:outline focus-visible:outline-2 focus-visible:outline-omd-ops">Menu</summary>
            <nav className="max-h-[70vh] overflow-y-auto px-4 pb-4 text-sm" aria-label="Admin navigation">
            {visibleGroups.map((group) => <details key={`${group.label}-mobile`} className="border-b border-slate-100 py-1"><summary className="cursor-pointer py-2 font-semibold text-slate-700">{group.label}</summary><div className="grid gap-1 pb-2">{group.items.map((item) => (
              <Link
                key={`${item.href}-${item.label}-mobile`}
                href={item.href}
                className="flex min-w-0 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 font-semibold text-slate-700"
              >
                <Icon name={item.icon} />
                {item.label}
                {runtimeConfig.phase1UatMode && !isPhase1AdminNavigationHref(item.href) ? (
        ...
        <main className="mx-auto min-w-0 w-full max-w-7xl px-3 py-5 sm:px-4 sm:py-6">{children}</main>
      </div>
    </div>
  );
}
