...
export default async function AdminKundliPage({ searchParams }: PageProps) {
  ...
  const cards = [
    ...
    ["Report ready", queue.counts.reportReady, "REPORT_READY"], ["Consultation", queue.counts.consultation, "CONSULTATION_SCHEDULED"], ["Due soon", queue.counts.dueSoon, "DUE_SOON"], ["Overdue", queue.counts.overdue, "OVERDUE"]
  ] as const;
  return <div className="grid gap-6">
    ...
    <section className="grid gap-3 md:grid-cols-4 xl:grid-cols-7">{cards.map(([label, value, state]) => <Link key={state} href={`/admin/kundli?state=${state}`}><AdminPanel className="p-4"><p className="text-xs font-semibold uppercase text-slate-500">{label}</p><p className="mt-2 text-2xl font-semibold text-slate-950">{value}</p></AdminPanel></Link>)}</section>
    <AdminPanel><form className="grid gap-3 md:grid-cols-[180px_1fr_1fr_auto_auto_auto]"><select name="state" defaultValue={params.state ?? ""} className="h-10 rounded-md border border-slate-300 px-3 text-sm"><option value="">All operational states</option>{["AWAITING_ASSIGNMENT","ASSIGNED","IN_REVIEW","REPORT_READY","CONSULTATION_SCHEDULED","DUE_SOON","OVERDUE"].map((item) => <option key={item} value={item}>{statusLabel(item)}</option>)}</select><select name="guruji" defaultValue={params.guruji ?? ""} className="h-10 rounded-md border border-slate-300 px-3 text-sm"><option value="">All Gurujis</option>{practitioners.map((item) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</select><select name="packageId" defaultValue={params.packageId ?? ""} className="h-10 rounded-md border border-slate-300 px-3 text-sm"><option value="">All packages</option>{packages.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="customerSelected" value="1" defaultChecked={params.customerSelected === "1"}/>Customer request</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="conflict" value="1" defaultChecked={params.conflict === "1"}/>Conflict</label><button className="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Filter</button></form></AdminPanel>
