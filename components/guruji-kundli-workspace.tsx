...
export async function GurujiKundliDetail({ orderId }: { orderId: string }) {
  ...
  const canPrepare = ["ASSIGNED", "IN_REVIEW"].includes(order.status);
  return <div className="grid gap-6">
...
<div className="mt-4 flex flex-wrap gap-3">{order.status === "ASSIGNED" ? <form action={updateGurujiKundliWorkAction}><input type="hidden" name="orderId" value={order.id}/><input type="hidden" name="command" value="START"/><button className="rounded-md bg-omd-ops px-4 py-2 text-sm font-semibold text-white">Mark work started</button></form> : null}{order.status === "IN_REVIEW" ? <form action={updateGurujiKundliWorkAction}><input type="hidden" name="orderId" value={order.id}/><input type="hidden" name="command" value="REPORT_IN_PROGRESS"/><button className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold">Mark report preparation in progress</button></form> : null}{order.status === "IN_REVIEW" ? <form action={updateGurujiKundliWorkAction}><input type="hidden" name="orderId" value={order.id}/><input type="hidden" name="command" value="REPORT_READY"/><button className="rounded-md bg-omd-brown px-4 py-2 text-sm font-semibold text-white">Mark report ready for admin review</button></form> : null}</div><form action={updateGurujiKundliWorkAction} className="mt-4 grid gap-3"><input type="hidden" name="orderId" value={order.id}/><input type="hidden" name="command" value="NOTE"/><textarea name="note" required rows={3} placeholder="Internal work note" className="rounded-md border border-slate-300 px-3 py-2"/><button className="w-fit rounded-md border border-omd-ops px-4 py-2 text-sm font-semibold text-omd-ops">Add internal note</button></form>{assignment.internalNote ? <p className="mt-4 rounded-md bg-slate-50 p-3 text-sm">{assignment.internalNote}</p>
 : null}</AdminPanel>
        {checklist ? <AdminPanel><h2 className="text-lg font-semibold">Work checklist</h2><p className="mt-1 text-sm text-slate-600">{checklist.progressPercent}% complete Â· {checklist.requiredPendingCount} required pending</p>
<div className="mt-4 grid gap-3">{checklist.items.map((item) => { const mayUpdate = !item.assignedUserId || item.assignedUserId === user.id || item.assignedRole?.toUpperCase().includes("ASTROLOGER"); return <div key={item.id} className="rounded-md border border-slate-200 p-3"><div className="flex justify-between gap-2"><p className="font-semibold">{item.title}</p>
<StatusBadge tone={statusTone(item.status)}>{statusLabel(item.status)}</StatusBadge></div>{item.description ? <p className="mt-1 text-sm text-slate-600">{item.description}</p>
 : null}{mayUpdate ? <form action={updateGurujiKundliChecklistItemAction} className="mt-3 grid gap-2 md:grid-cols-[150px_1fr_auto]"><input type="hidden" name="orderId" value={order.id}/><input type="hidden" name="itemId" value={item.id}/><select name="status" defaultValue={item.status} className="h-10 rounded-md border border-slate-300 px-3 text-sm">{["pending","in_progress","blocked","completed"].map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select><input name="note" defaultValue={item.internalNote ?? item.blockedReason ?? ""} placeholder="Internal checklist note" className="h-10 rounded-md border border-slate-300 px-3 text-sm"/><button className="rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold">Save</button></form> : <p className="mt-2 text-xs text-slate-500">Read-only admin-owned task.</p>
