...
export function CollectionCard({
  ...
  return (
    <Link href={href} className="group block overflow-hidden rounded-2xl border border-omd-sand bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-[#b00000] hover:shadow-lg">
      <div
      ...
      >
        {!imageUrl ? (
          <div className="flex h-full items-center justify-center px-4 text-center">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-[#b00000]">OMDivyaDarshan</p>
      ...
      <div className={compact ? "p-3" : "p-5"}>
        <p className="text-xs font-semibold uppercase tracking-wide text-[#b00000]">Collection</p>
        <h3 className={`${compact ? "mt-1 line-clamp-2 text-sm leading-5" : "mt-2 text-xl"} font-semibold text-omd-brown group-hover:text-[#b00000]`}>{title}</h3>
        {description && !compact ? <p className="mt-2 line-clamp-2 text-sm leading-6 text-omd-muted">{description}</p> : null}
        <p className={`${compact ? "mt-2 text-xs" : "mt-4 text-sm"} font-semibold text-[#b00000]`}>{cta}</p>
      </div>
    </Link>
  );
}
